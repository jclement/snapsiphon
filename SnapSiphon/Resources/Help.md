...
- **Hidden album** (optional, off by default): hidden photos are often the most sensitive, so they're excluded unless you opt in. **iOS caveat**: since iOS 16, apps can only see the Hidden album when its Face ID lock is off (Settings → Photos → *Use Face ID*). With the lock on — the iOS default — hidden photos are invisible to SnapSiphon no matter what: they can't be backed up, and hiding an already-backed-up photo looks like deletion (it's tombstoned, protected by the grace period, and resurrected automatically if you unhide it).
