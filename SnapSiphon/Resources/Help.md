...
- A one-tap **verify** that checks every expected blob is present with the expected size.
...
## Exactly one writer per folder

- **Multi-device writing is not supported. Two devices must never back up into the same folder** — give each its own prefix (or bucket). The checks below are safety fuses, not a coordination protocol.
- Pointing a fresh install at a folder that already holds a repository raises a prompt: **take over** (reload the index from the bucket — right after a reinstall or when the old phone is retired), **verify match first** (read-only comparison against the actual Apple Photos library, plus remote blob presence/size checks), or **use a different folder**. Exact PhotoKit IDs are checked first; unique filename + capture-time + media-type metadata provides a best-effort match if IDs changed during migration. Limited Photos access or a Face ID-locked Hidden album can make unmatched results inconclusive.
- If the repository advances unexpectedly, falls behind local history, has a chain gap, or a foreign journal replaces the expected head, backups stop with a hard conflict. Providers supporting conditional PUTs enforce journal create-once atomically; other providers get best-effort head/read-back checks.
...
- **Hidden album** (optional, off by default): hidden photos are often the most sensitive, so they're excluded unless you opt in. **iOS caveat**: since iOS 16, apps can only see the Hidden album when its Face ID lock is off (Settings → Photos → *Use Face ID*). The practical recipe: turn the lock off, run a backup, turn it back on. Items backed up as hidden are recorded as hidden in the repository, so **re-locking never marks them deleted** — they're simply retained. (A photo hidden *before* it was ever backed up, with the lock on, is invisible to SnapSiphon and looks like a deletion if it was previously visible — tombstoned, grace-protected, and resurrected automatically if it reappears.)
...
1. **Restore script** (Settings → Disaster recovery): one Python file. `python3 restore.py` shows its configuration for review (ENTER to confirm, or pick a number to change a value), reads the newest readable checkpoint, replays the journals, and rebuilds everything with original filenames and SHA-256 integrity checks — including verified resume of existing output files. Export it **with secrets baked in** (one file that just works — store it like a password) or **without secrets** (the script prompts for the bucket secret key and X25519 age secret at run time — safe to keep anywhere). Needs the `age` CLI *or* `pip3 install cryptography` (on a Mac, the system `python3` or Homebrew's — a python.org install needs its "Install Certificates.command" run once first). The embedded Python backend supports X25519; hardware/plugin identities require the `age` CLI and their corresponding plugin.
...
Any S3-compatible provider works — Backblaze B2, Cloudflare R2, AWS, Wasabi, MinIO, or fully self-hosted with [picos3](https://github.com/jclement/picos3) over Tailscale (compose file in the repo's docs/). The Storage screen has a per-provider cheat sheet for endpoints and regions (paste the endpoint with or without `https://` — it's cleaned up on save; a bucket name containing a dot needs path-style addressing on AWS and Wasabi). What makes a *great* bucket:

- **Append-only key** — SnapSiphon only needs read/write/list for backups. Delete permission is used only by garbage collection — version deletion on versioned buckets, ordinary object deletion on unversioned ones. A key that cannot delete means malware or a stolen phone cannot physically erase the archive; cleanup reports “blocked” until retention expires or you deliberately use a fuller key.
